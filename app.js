...
let lastCoordinatorHeartbeat = 0;
let controlPause = false;
let controlOpenEarly = false;
let ticketCounter = 0; // Глобальный счётчик для админа
...
// Создаём экран администратора динамически, если его нет в index.html
function ensureAdminScreen() {
    if (!document.getElementById('screen-admin')) {
        ...
        adminScreen.innerHTML = `
            <div id="admin-event-name" class="queue-label"></div>
            <div id="admin-queue-status" class="queue-label"></div>
            <button id="btn-open-early" class="action-button">Открыть очередь досрочно</button>
            <button id="btn-pause" class="action-button">Пауза</button>
            <button id="btn-resume" class="action-button">Возобновить</button>
            <button id="btn-admin-participants" class="action-button">Список участников</button>
        `;
...
document.getElementById('btn-back').addEventListener('click', () => showScreen(isAdmin ? 'screen-admin' : 'screen-queue'));
document.getElementById('btn-update-list').addEventListener('click', showParticipants);

// Кнопки администратора
document.getElementById('btn-open-early').addEventListener('click', openEarly);
...
document.getElementById('btn-admin-participants').addEventListener('click', showParticipants);
...
// Подключение к MQTT
function connectMQTT(asAdmin = false) {
    ...
    mqttClient.on('connect', () => {
        ...
        mqttClient.subscribe(`${base}/coordinator/status`);
        mqttClient.subscribe(`${base}/control`);
    });
    ...
    mqttClient.on('close', () => {
        ...
        setTimeout(() => {
            if (selectedCity) connectMQTT(isAdmin);
        }, 5000);
    });
...
function publishWithRateLimit(topic, payload, callback, qos = 0) {
    const now = Date.now();
...
function handleMessage(msgType, message) {
    if (msgType === 'join') {
    ...
    } else if (msgType === 'coordinator/status') {
        coordinatorActive = true;
        lastCoordinatorHeartbeat = Date.now();
        ...
        if (action === 'pause_queue') {
            ...
            controlPause = false;
        } else if (action === 'open_queue_early') {
            controlOpenEarly = true;
        }
    }
}
...
function loadSnapshot(snapshot, isFromCoordinator = false) {
    if (!snapshot || !snapshot.queue) return;
    const incomingVersion = snapshot.version || 0;

    if (!isFromCoordinator && incomingVersion <= snapshotVersion) return;

    if (incomingVersion > snapshotVersion) {
        snapshotVersion = incomingVersion;
    }

    const newParticipants = [];
    snapshot.queue.forEach(p => {
        if (!leftClients.has(p.client_id)) {
            newParticipants.push({ ...p });
        }
    });
    participants = newParticipants;
    lastChangeTime = Date.now();
    // ВАЖНО: если мы ожидали входа, и теперь наш ID в списке, снимаем isJoining
    if (participants.some(p => p.client_id === myClientId)) {
        isJoining = false;
    }
}
...
function publishSnapshot() {
    ...
    publishWithRateLimit(`event/${currentEventId}/${currentCity}/snapshot`, JSON.stringify(snapshot), null, 0);
}
...
function joinQueue() {
    ...
    publishWithRateLimit(`event/${currentEventId}/${currentCity}/join`, JSON.stringify(joinMsg), () => {
        isSending = false;
        // Если координатор активен, ждём его снапшот, не добавляем локально
    ...
    // Дополнительно сразу вызываем обновление интерфейса
    updateQueueUI();
}
...
function checkCoordinatorTimeout() {
    if (coordinatorActive && (Date.now() - lastCoordinatorHeartbeat > 15000)) {
...
function updateQueueUI() {
    ...
    document.getElementById('queue-timer').textContent = now < start && !controlOpenEarly ? `До начала: ${hours}:${minutes}:${seconds}` : 'Очередь открыта';
    ...
    if (controlPause) {
        ...
        joinBtn.textContent = 'Очередь на паузе';
        return;
    }
...
function updateAdminUI() {
    const eventNameElem = document.getElementById('admin-event-name');
    if (eventNameElem) {
        eventNameElem.textContent = EVENT.name;
    }
    const queueStatusElem = document.getElementById('admin-queue-status');
    if (queueStatusElem) {
        if (controlPause) {
            queueStatusElem.textContent = 'Статус очереди: пауза';
        } else if (controlOpenEarly) {
            queueStatusElem.textContent = 'Статус очереди: открыта досрочно';
        } else {
            queueStatusElem.textContent = 'Статус очереди: открыта';
        }
    }
}

function sendHeartbeat() {
    if (!selectedEvent || !selectedCity) return;
    const topic = `event/${selectedEvent.id}/${selectedCity}/coordinator/status`;
    const payload = JSON.stringify({ active: true, ts: Date.now() / 1000 });
    if (mqttClient && mqttClient.connected) {
...
function openEarly() {
    if (!selectedEvent || !selectedCity) return;
    const topic = `event/${selectedEvent.id}/${selectedCity}/control`;
    const payload = JSON.stringify({ action: 'open_queue_early' });
    if (mqttClient && mqttClient.connected) {
        mqttClient.publish(topic, payload, { qos: 1, retain: true });
    }
    controlOpenEarly = true;
...
function pauseQueue() {
    if (!selectedEvent || !selectedCity) return;
    const topic = `event/${selectedEvent.id}/${selectedCity}/control`;
    const payload = JSON.stringify({ action: 'pause_queue' });
    if (mqttClient && mqttClient.connected) {
        mqttClient.publish(topic, payload, { qos: 1, retain: true });
    }
    controlPause = true;
...
function resumeQueue() {
    if (!selectedEvent || !selectedCity) return;
    const topic = `event/${selectedEvent.id}/${selectedCity}/control`;
    const payload = JSON.stringify({ action: 'resume_queue' });
    if (mqttClient && mqttClient.connected) {
        mqttClient.publish(topic, payload, { qos: 1, retain: true });
    }
    controlPause = false;
